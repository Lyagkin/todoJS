...
function savePositionTask() {
  ...
  tasks.forEach((item, index) => {
    ...
    const taskIndex = arrayTasksLocalStorage.findIndex(item => item.id === taskId);
    if(taskIndex !== -1) arrayTasksLocalStorage[taskIndex].position = index; 
  });
...
export function initSortableList(e) {
  ...
  let nextSibling = siblings.find(item => e.clientY <= item.offsetTop + item.offsetHeight/2);

  output.insertBefore(draggingTask, nextSibling);
}